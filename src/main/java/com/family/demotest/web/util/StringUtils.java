...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
	...
	/**
	 * 驼峰式  将 parent_id 或 parent.id 转换成  parentId
	 * @param str
	 * @return
	 */
	public static final String toHump(String str) {
		String split = null;
		if(isEmpty(str)) return str;
		
		split = contains(str, "_") ? "_" :  contains(str, ".") ? "." : null;
		
		if(isEmpty(split)) return str;
		
		String[] sArr = split(str, split);
		
		String prop = sArr[0];
		
		for(int i = 1, len = sArr.length; i < len; i ++ ) {
			prop += String.valueOf(sArr[i].charAt(0)).toUpperCase() + sArr[i].substring(1);
		}
		return prop;
	...
	public static Map<String,String> str2map(String string){
		...
		Map<String,String> str2map=new HashMap<>();
		for (String str : strs) {
    ...
    public static String addCharInFront(String str,int length,char ch){
    	int len = str.length();
    	if(length<len){
    		return str;
    	}
