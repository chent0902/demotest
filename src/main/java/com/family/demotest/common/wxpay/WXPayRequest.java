...
public class WXPayRequest
{
     ...
     * @throws Exception
     */
    private String requestOnce(final String domain, String urlSuffix, String uuid, String data, int connectTimeoutMs, int readTimeoutMs, boolean useCert)
        throws Exception
    {
        ...
        httpPost.addHeader("User-Agent", "wxpay sdk java v1.0 "+config.getMchID()); // TODO:
                                                                                    // 很重要，用来检测
                                                                                    // sdk
                                                                                    // 的使用情况，要不要加上商户信息？
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            WXPayReport.getInstance(config).report(uuid, elapsedTimeMillis, domainInfo.domain, domainInfo.primaryDomain, connectTimeoutMs, readTimeoutMs,
        ...
        throws Exception
    {
         ...
         * 
         * boolean useCert = false; try { result = requestOnce(domain,
         * urlSuffix, uuid, data, connectTimeoutMs, readTimeoutMs, useCert);
         * return result; } catch (UnknownHostException ex) { // dns 解析错误，或域名不存在
         * exception = ex; WXPayUtil.getLogger().warn(
         * "UnknownHostException for domain {}, try to use {}", domain,
         * this.primaryDomain); shouldRetry = true; } catch
         * (ConnectTimeoutException ex) { exception = ex;
         * WXPayUtil.getLogger().warn
         * ("connect timeout happened for domain {}, try to use {}", domain,
         * this.primaryDomain); shouldRetry = true; } catch
         * (SocketTimeoutException ex) { exception = ex; shouldRetry = false; }
         * catch (Exception ex) { exception = ex; shouldRetry = false; }
         * 
         * if (shouldRetry) { result = requestOnce(this.primaryDomain,
         * urlSuffix, uuid, data, connectTimeoutMs, readTimeoutMs, useCert);
        ...
        throws Exception
    {
         ...
         * String result; Exception exception; boolean shouldRetry = false;
         * 
         * boolean useCert = true; try { result = requestOnce(domain, urlSuffix,
         * uuid, data, connectTimeoutMs, readTimeoutMs, useCert); return result;
         * } catch (ConnectTimeoutException ex) { exception = ex;
         * WXPayUtil.getLogger().warn(String.format(
         * "connect timeout happened for domain {}, try to use {}", domain,
         * this.primaryDomain)); shouldRetry = true; } catch
         * (SocketTimeoutException ex) { exception = ex; shouldRetry = false; }
         * catch (Exception ex) { exception = ex; shouldRetry = false; }
         ...
         * requestOnce(this.primaryDomain, urlSuffix, uuid, data,
         * connectTimeoutMs, readTimeoutMs, useCert, autoReport); return result;
     ...
     * @throws Exception
     */
    public String requestChange(String curl, String uuid, String data, int connectTimeoutMs, int readTimeoutMs, boolean useCert)
        throws Exception
    {
        BasicHttpClientConnectionManager connManager;
        if(useCert)
